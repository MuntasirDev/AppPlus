// src/pages/AppDetail.jsx (Refactored to Pure HTML/Tailwind)

import React, { useState, useEffect, useCallback } from 'react';
import { useParams, useNavigate, useLoaderData } from 'react-router-dom';
import { Star, Download, CheckCircle, User } from 'lucide-react';

// --- PLACEHOLDER HOOKS (Use your actual hooks if available) ---
const useToast = () => ({
...
// Component for rendering display-only stars
...
// Component for the interactive star picker
        ...
        {[1, 2, 3, 4, 5].map((star) => (
            <Star 
                ...
                className={`h-6 w-6 cursor-pointer transition ${
                    star <= rating ? 'fill-amber-400 text-amber-400' : 'text-gray-500 hover:text-amber-300'
                }`}
            />
        ))}
    </div>
...
// --- CUSTOM HTML/TAILWIND COMPONENTS TO REPLACE UI LIBRARY ---

const CustomCard = ({ children, className = '' }) => (
    <div className={`bg-gray-800 rounded-xl shadow-md border border-gray-700 ${className}`}>
        {children}
    </div>
);
                                ...
                                <ul className="space-y-3 text-gray-300">
                                    {app.features.map((feature, index) => (
                                        <li key={index} className="flex items-start gap-2">
                                            {/* Changed plain bullet to CheckCircle for style */}
                                    ...
                                    allReviews.map((review, index) => (
                                        <div key={index} className="border-b border-gray-700 pb-4 last:border-0">
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                <span className="font-semibold text-teal-300">{review.user}</span>
                        ...
                        <CustomCard className="sticky top-20">
                            ...
                            <CustomCardContent>
                                <form onSubmit={handleSubmitReview} className="space-y-5">
                                    ...
                                    {/* Rating Input (Replacing <Label> & <Input>) */}
                                    <div className="space-y-2">
                                        <label htmlFor="rating" className="block text-sm font-medium text-gray-300">Rating (1-5)</label>
                                        <InteractiveStars rating={reviewRating} setRating={setReviewRating} />
                                    </div>

                                    {/* Review Textarea (Replacing <Label> & <Textarea>) */}
                                    <div className="space-y-2">
